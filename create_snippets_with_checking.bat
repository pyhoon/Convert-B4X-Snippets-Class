...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $lines=@(Get-Content -Path '%INPUT%' -ErrorAction Stop); $idx=$lines | %%{$i=0}{if($_ -match 'Sub (Class|Process)_Globals'){$i;break} else {$i++}}; if($idx -ne $null){$snippet=$lines[($idx+1)..($lines.Count-1)]; if($snippet.Count -gt 0 -and $snippet[-1].Trim() -eq 'End Sub'){$snippet=$snippet[0..($snippet.Count-2)]}; $snippet | Set-Content -Path '%OUTPUT%' -ErrorAction Stop; Write-Host 'Snippet written.'} else {Write-Host 'Marker not found.'}"
